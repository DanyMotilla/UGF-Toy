...
// From: drawing.glsl:66
float mix11(float a, float b, float t) {
    return mix(a, b, t * 0.5 + 0.5);
}

float mix11_smooth(float a, float b, float t) {
    float smoothT = smoothstep(0.0, 1.0, t * 0.5 + 0.5);
    return mix(a, b, smoothT);
}

float mix11_cubic(float a, float b, float t) {
    float cubicT = t * t * (3.0 - 2.0 * t);
    float finalT = cubicT * 0.5 + 0.5;
    return mix(a, b, finalT);
}

// From: drawing.glsl:198
